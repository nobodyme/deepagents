...
- Remote sandbox provider internals (Daytona, LangSmith, Modal, Runloop, Vercel, AgentCore) — third-party. The local `docker` provider is first-party shipped code and is NOT covered by this exclusion; see assumption 4.
...
4. Remote sandbox backends are trusted third-party services. CLI responsibility ends at correctly constructing and dispatching requests to them. The local `docker` provider is different: the CLI itself creates and destroys containers on the user's Docker daemon, and isolation is container-level only (shared host kernel; access to the Docker daemon is generally root-equivalent). It is intended for local development and testing and is not a substitute for a remote sandbox when working with untrusted repositories.
...
| C7  | Sandbox Integration         | Creates/destroys sandboxes — remote (Daytona, LangSmith, Modal, Runloop, Vercel, AgentCore) or local Docker containers | framework-controlled | No⁴      | `integrations.sandbox_factory.create_sandbox`, `integrations.docker_sandbox.DockerSandbox`        |
...
| Remote sandbox provider security vulnerabilities                       | Daytona, LangSmith, Modal, Runloop, Vercel, and AgentCore are third-party services. Their internal security is not this project's responsibility. The local `docker` provider is first-party but delegates isolation to the user's Docker daemon: container-level (shared-kernel) isolation only, weaker than a remote sandbox. | Correctly initializing sandbox sessions via `integrations.sandbox_factory.create_sandbox`; documenting that the local `docker` provider is not a remote-isolation substitute for untrusted repositories. |
